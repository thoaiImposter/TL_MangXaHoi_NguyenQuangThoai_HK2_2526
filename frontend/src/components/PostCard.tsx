...
import type { CommentMedia, PostComment, PostFeedItem, PostMedia, User } from '../types';
...
type PostCardProps = {
  ...
  onBeginEdit: (post: PostFeedItem) => void;
  ...
  onSetCommentDraft: Dispatch<SetStateAction<Record<number, Draft>>>;
  onSetReplyDrafts: Dispatch<SetStateAction<Record<number, Draft>>>;
  onSetReplyTarget: Dispatch<SetStateAction<Record<number, number | null>>>;
            ...
            {replyTarget[post.id] === comment.id && (
              <div className="comment-reply-box">
                ...
                <div className="feed-actions">
                  ...
                  <button className="primary-button" type="button" onClick={() => onSubmitReply(post.id, comment.id)}>
                    Gửi
                  </button>
                </div>
              </div>
  ...
  return (
    <article className={`post ${expandedPost ? 'post-expanded' : ''}`} style={{ cursor: 'pointer' }} onClick={handlePostClick}>
      <div className="post-head">
        ...
        {post.authorId === user.id && (
          <div className="feed-actions post-card-tools">
            <button className="chip" type="button" onClick={() => onBeginEdit(post)}>
              Sửa
            </button>
            <button className="chip" type="button" onClick={() => onRemove(post.id)}>
              Xóa
            </button>
          </div>
        )}
        {post.authorId !== user.id && (
          <div className="feed-actions post-card-tools">
            <ReportButton reporterId={user.id} targetType="post" targetId={post.id} />
          </div>
        )}
      </div>
      ...
      {post.content.length > 180 && (
        <button className="chip" type="button" onClick={() => onToggleExpandedPost(post.id)}>
          {expandedPost ? 'Thu gọn' : 'Xem thêm'}
        </button>
      )}

      {post.isPoll && (
        <PollCard post={post} userId={user.id} />
      )}

      <PostMediaSection media={post.media} />

      <div className="feed-actions post-actions">
        <button className={`chip ${post.likedByMe ? 'chip-solid' : ''}`} type="button" onClick={() => onLikePost(post.id)}>
          Thích <span>{post.likeCount}</span>
        </button>
        <button className="chip" type="button" onClick={() => onToggleExpandedComments(post.id)}>
            ...
            <textarea
              ...
              onChange={(e) =>
                onSetCommentDraft((current) => ({
                  ...current,
                  [post.id]: { content: e.target.value, media: current[post.id]?.media ?? [] },
                }))
              }
            />
            <CommentDraftMedia
              ...
              onRemove={() => onSetCommentDraft(current => ({
                ...current,
                [post.id]: { content: current[post.id]?.content ?? '', media: [] },
              }))}
            />
            <div className="feed-actions">
              <label className="icon-upload">
                <input type="file" accept="image/*,video/*,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.zip,.rar,.7z,.txt,.csv" onChange={(e) => onPickCommentMedia(post.id, e.target.files)} />
                <svg viewBox="0 0 24 24" aria-hidden="true">
                  <path
                    d="M4 7a3 3 0 013-3h6.5c.8 0 1.5.3 2.1.9l3.5 3.5c.6.6.9 1.3.9 2.1V17a3 3 0 01-3 3H7a3 3 0 01-3-3V7zm8 0v3h3"
                    fill="none"
              ...
              <button className="primary-button" type="button" onClick={() => onSubmitComment(post.id)}>
                Gửi
              </button>
            </div>
          </div>
